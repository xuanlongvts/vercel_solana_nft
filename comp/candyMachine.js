...
const config = new web3.PublicKey(publicRuntimeConfig.CANDY_MACHINE_CONFIG);
...
const CandyMachine = ({ walletAddress }) => {
    ...
    const getProvider = () => {
        const rpcHost = publicRuntimeConfig.SOLANA_RPC_HOST;

        // Create a new connection object
        const connection = rpcHost && new Connection(rpcHost);
    ...
    const getCandyMachineState = async () => {
        ...
        const data = await fetchHashTable(publicRuntimeConfig.CANDY_MACHINE_ID, true);
        if (data.length) {
        ...
        const candyMachine = await program.account.candyMachine.fetch(publicRuntimeConfig.CANDY_MACHINE_ID);
    ...
    const fetchHashTable = async (hash, metadataEnabled) => {
        const connection = new web3.Connection(publicRuntimeConfig.SOLANA_RPC_HOST);
            ...
            <div className="gif-grid">
                {mints.map(mint => (
                    <div className="gif-item" key={mint}>
                        <Image src={mint} alt={`Minted NFT ${mint}`} quality={100} width={236} height={236} />
